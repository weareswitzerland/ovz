...
function showLocationOnMap(locationId) {
    ...
    if (location) {
        // Wait slightly for view transition
        setTimeout(() => {
            mapInstance.flyTo([location.lat, location.lng], 18, {
                animate: true,
                duration: 1.5
            });
...
function openModal(bird) {
    ...
    modalCategory.textContent = bird.category;
    ...
    if (bird.image) {
        ...
        // Optional: Set a placeholder or keep hidden
    }
    ...
    if (bird.ebirdLink) {
        modalEbirdBtn.href = bird.ebirdLink;
        modalEbirdBtn.style.display = 'inline-flex';
    } else {
        modalEbirdBtn.style.display = 'none';
    }

    currentBirdLocationId = bird.locationId;
    modal.showModal();
}
